...
checkIfPointsAreInSBF <- function(grid, n, S, H, b_vector, df_test_points){
  ...
  for (i in 1:length(df_test_points[,1])) {
    cell_and_result <- checkIfPointIsInSBF(grid,
...
testPoints <- function(df_test_points, grid, input, output, is_initial = FALSE){
...
printMessage <- function(message, output, type_of_msg){
  if (type_of_msg == "error") {
    ...
    output$error_text <- renderText({
      paste("<font color=\"#FF0000\">","Following error occured:\n",message)
    })
...
renderInitialMap <- function(input, output, session, worldmap, parameters){
  
  loadParametersFromCSV(input, output, session)
  
  grid <- createGrid(parameters$country, parameters$cell_size, worldmap)
  
  df_test_point <- data.frame(lat_test = parameters$lat_test,
                              lon_test = parameters$lon_test,
                              name = "test point")
  
  input = list ("lon_poi" = parameters$lon_poi,
    ...
    error <- tryCatch(
      {
        ...
        updateSelectInput(session, "algorithm",
                          choices = algorithms,
        ...
        updateSelectInput(session, "randomGenerationModes", 
...
saveParametersToCSV <- function(input, output){
  parameters <- data.frame("country" = input$country,
