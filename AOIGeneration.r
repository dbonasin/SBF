...
coverageAOI <-
  function(grid,
    ...
    rasterizedGrid <-
      fasterize::raster(grid,
                        resolution = c(size_of_cell, size_of_cell),
                        crs = 4326)
    ...
    grid$intersects_buffer <-
      ifelse(sf::st_intersects(grid, buffer, sparse = F),
             "Yes",
             "No")
    
    # does the cell have center
    grid$has_center <-
      ifelse(sf::st_intersects(grid, point, sparse = F),
             "Yes",
             "No")
...
partitionAOI <- function(S, partition_count) {
  ...
  for (i in 1:partition_count) {
    if (m != 0) {
      S[which(S$manDist >= (max_distance - partition_length) &
                S$manDist <= max_distance), ]$label <- i
      
      max_distance <- max_distance - (partition_length + 1)
      m <- m - 1
    } else {
      S[which(S$manDist >= (max_distance - partition_length + 1) &
                S$manDist <= max_distance), ]$label <- i
